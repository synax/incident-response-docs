...
description: This is an open-source version of Synax's Incident Response training course. What started as an internal course to train new Incident Commanders at Synax has since developed into training that we now deliver publicly. Learn more about incident response and the role of an Incident Commander.
pdf: /assets/pdf/synax_incident_response_training_public.pdf
---

!!!info "Incident Response Training Course"
    This is an open-source version of "Incident Response Training", our Synax training course for incident response and incident command. It started as an internal course to train new Incident Commanders and has since developed into one that we now deliver publicly.

    It includes lots of introductory information on our process, and details on the Incident Commander role specifically. All the information is already available as part of our [public documentation](https://response.synax.io) , this is just a different way of presenting it that's hopefully more engaging. Feel free to use this as a base for training in your own organization.
...
The goal of this session is to give you an understanding of how to effectively manage incidents within your organization. I'll describe the process we use at Synax for managing critical incidents, and talk in more detail about a specific role called the "Incident Commander".

This isn't a sales pitch. I'm not on our sales team, and this isn't a talk about how to use Synax to manage your incidents (although obviously it would be awesome if you did). The intent today is to introduce you to how we manage incidents internally at Synax, and provide you with lots of practical information you can take away to your own organizations to either start or improve your own incident response processes.
...
Here is Synax’s definition of an incident,
...
We also have something we call a **major incident**. This is any incident where we require a coordinated response between teams. Again, this is just our definition at Synax, feel free to use your own.
...
So let’s talk about our process a bit more. The way we do incident response at Synax isn’t something we invented ourselves, it is heavily based on the [Incident Command System](https://en.wikipedia.org/wiki/Incident_Command_System), usually abbreviated to ICS.
...
NIMS and ICS are the basis of the process we use at Synax, however we have heavily modified it for our needs. Turns out that things can be streamlined a bit when human life isn't on the line.
...
When developing our process at Synax, we looked at a few of the other systems in use around the world, and chose the bits we liked the most to add to our own.
...
Whether this works for you will depend on your organization. This is how we do it at Synax and it works well for us, but I can imagine it not being easy to get this sort of buy-in in other organizations. I used to work in the airline industry, and I don't think this rule would fly there (Get it? Airline industry. Fly. Hello? This thing on?).
...
This one definitely wasn't said by anyone at Synax ever. Nope. Definitely not.
...
Another one that's definitely never been mentioned on any Synax incident response call ever.
...
Oh look, it's another one that's never ever happened at Synax. We used to have a really big problem with this one. We'd start incident response calls, then spend the first 10 minutes arguing over whether it was a `SEV-3` or a `SEV-2`. By the time we were finished, we would be 10 minutes into a `SEV-1` and have made no progress.
...
Believe it or not, we used to page every single engineer at Synax whenever we had a `SEV-2`. I'm not joking. It was horrible. It worked great when we only had 5 engineers, less so when we had 50.
...
At Synax, we keep our internal updates to about once every 20-30 minutes. Writing the update takes away time from solving the incident, so that needs to be balanced. This cadence has worked well for us.
...
_084. Synax University._

???+ aside hide-arrow "Synax University"
    Shameless plug: If you're interested in our longer courses on this and other topics, including how to use Synax to do it, we offer a variety of different training programs as part of [Synax University](https://www.synax.io/university/) — from private full-day courses at your own offices, to public instructor-led training.
...
???+ aside hide-arrow full-width "Synax Summit Series Chicago 2017"
    As a special bonus for making it to the end, here's a recording of an earlier version of this training given at a Synax event in September 2017. The material will differ slightly from that shown on this website, as we have made changes and refined the content since then. But it should give you a decent idea of how the course is usually presented.
